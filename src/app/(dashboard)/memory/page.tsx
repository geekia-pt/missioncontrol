...
const Memory: React.FC = () => {
  ...
  const [workspaceMemories, setWorkspaceMemories] = useState<WorkspaceMemory[]>([]);
  const [expandedMemory, setExpandedMemory] = useState<string | null>(null);

  useEffect(() => {
    fetch('/api/memory')
      .then(r => r.json())
      .then(d => setWorkspaceMemories(d.memories || []))
      .catch(() => {});
  }, []);
  ...
  return (
    <div style={styles.page}>
      ...
      {/* ── OpenClaw Workspace Memory (live) ── */}
      {workspaceMemories.length > 0 && (
        <motion.div style={styles.categorySection} {...stagger(5)}>
          <div style={styles.sectionHeader}>
            ...
            <span style={styles.sectionMeta}>{workspaceMemories.length} ficheiros</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {workspaceMemories.map((mem, i) => (
              <div key={i} style={{
                ...
                <button
                  ...
                  onClick={() => setExpandedMemory(expandedMemory === `${mem.agentId}-${mem.file}` ? null : `${mem.agentId}-${mem.file}`)}
                >
                  <span style={{ fontSize: 16 }}>🧠</span>
                  <div style={{ flex: 1, textAlign: 'left' as const }}>
                    <div style={{ fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-body)' }}>{mem.agentId}</div>
                    <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-tertiary)' }}>{mem.file} · {mem.size} chars</div>
                  </div>
                  <ChevronRight size={14} color="var(--text-tertiary)" style={{ transform: expandedMemory === `${mem.agentId}-${mem.file}` ? 'rotate(90deg)' : 'none', transition: 'transform 0.15s' }} />
                </button>
                {expandedMemory === `${mem.agentId}-${mem.file}` && (
                  <div style={{
                    padding: '0 14px 14px 14px',
                    fontFamily: 'var(--font-mono)', fontSize: 11,
                    color: 'var(--text-secondary)', whiteSpace: 'pre-wrap',
                    borderTop: '1px solid var(--border-subtle)', paddingTop: 10, marginTop: 0,
