...
interface HubData {
  sessions: { id: string; status: string; model: string; channel: string; tokens: number; cost: number; startedAt: string }[]
  totalTokens: number
  totalCost: number
  vitals: { cpu: number; memory: number; uptime: number }
  gatewayUp: boolean
}
...
const agents: LiveAgent[] = []; // fallback empty
...
const Hub: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/api/hub').then(r => r.json()).then(setHubData).catch(() => {});
    fetch('/api/agents').then(r => r.json()).then(d => setLiveAgents(d.agents || [])).catch(() => {});
  }, []);

  // Use live agents if available, fallback to empty
  const agentList = liveAgents.length > 0 ? liveAgents : agents;
        ...
        <div style={styles.headerStats}>
          ...
          {hubData && (
            <>
              <div style={{
                ...styles.statChip,
                borderColor: hubData.gatewayUp ? 'var(--status-online)' : 'var(--border-subtle)',
                color: hubData.gatewayUp ? 'var(--status-online)' : 'var(--text-tertiary)',
              }}>
                <Activity size={10} />
                <span>Gateway {hubData.gatewayUp ? 'online' : 'offline'}</span>
              </div>
              {hubData.sessions.length > 0 && (
                <div style={styles.statChip}>
                  <div style={{ ...styles.statDot, background: 'var(--marketing-color)' }} />
                  <span>{hubData.sessions.length} sessões</span>
                </div>
              )}
            </>
          )}
              ...
              >
                {selectedAgent ? (
                  <span style={styles.selectedAgentDisplay}>
                    <span style={{ fontSize: '16px' }}>{(selectedAgent as LiveAgent).emoji || '🤖'}</span>
                ...
                {showAgentDropdown && (
                  ...
                  >
                    {agentList.map(agent => (
                      ...
                      >
                        <span style={{ fontSize: '16px' }}>{(agent as LiveAgent).emoji || '🤖'}</span>
                        <div style={{ flex: 1 }}>
