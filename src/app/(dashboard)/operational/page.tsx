...
const Operational: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/api/jobs')
      .then(r => r.json())
      .then(d => { setJobs(d.jobs || []); setGatewayUp(true); })
    ...
    <div style={styles.page}>
      ...
      <div style={{ padding: '0 24px 16px', borderBottom: '1px solid var(--border-subtle)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '1px' }}>
            Gateway Jobs
          </span>
          {!gatewayUp && (
            <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>
        ...
        {jobs.length === 0 ? (
          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>
            {gatewayUp ? 'Sem cron jobs configurados.' : 'Gateway offline — cron jobs indisponíveis.'}
          </div>
        ) : (
            ...
            {jobs.map(job => (
              <div key={job.id} style={{
                ...
                <div style={{
                  ...
                  background: job.status === 'active' ? 'var(--status-online)' : job.status === 'running' ? 'var(--accent-cyan)' : job.status === 'failed' ? 'var(--status-error)' : 'var(--text-tertiary)',
                }} />
                <span style={{ fontSize: 11, fontFamily: 'var(--font-body)', color: 'var(--text-primary)', fontWeight: 600 }}>{job.name}</span>
                <span style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-tertiary)', background: 'var(--bg-hover)', padding: '2px 6px', borderRadius: 4 }}>{job.schedule}</span>
                {job.nextRun && <span style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-tertiary)' }}>next: {job.nextRun}</span>}
                <button
                  style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 'var(--radius-sm)', border: 'none', background: 'var(--accent-cyan-dim)', color: 'var(--accent-cyan)', fontSize: 10, fontWeight: 600, cursor: 'pointer' }}
